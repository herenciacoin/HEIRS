...
*Setup instructions for a gitian build of HERENCIA Core using a VM or physical system.*

Gitian is the deterministic build process that is used to build the HERENCIA
...
to the HERENCIA GitHub release page.
...
- [Building HERENCIA Core](#building-herencia-core)
...
```bash
gpg --output ${VERSION}-linux/${NAME}/herencia-linux-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-linux/$NAME/herencia-linux-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-osx-unsigned/$NAME/herencia-osx-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-osx-unsigned/$NAME/herencia-osx-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-win-unsigned/$NAME/herencia-win-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-win-unsigned/$NAME/herencia-win-${VERSION%\.*}-build.assert
```

Uploading Signatures
--------------------
Make a Pull Request (both the `.assert` and `.assert.sig` files) to the
[gitian.sigs](https://github.com/herencia-project/gitian.sigs/) repository:
...
You can also mail the files to Fuzzbawls (fuzzbawls@herencia.org) and he will commit them.

```bash
gpg --detach-sign ${VERSION}-linux/${NAME}/herencia-linux-*-build.assert
gpg --detach-sign ${VERSION}-win-unsigned/${NAME}/herencia-win-*-build.assert
gpg --detach-sign ${VERSION}-osx-unsigned/${NAME}/herencia-osx-*-build.assert
```
