...
The build script we'll be using is contained in the HERENCIA github repository ([contrib/gitian-build.py](https://github.com/herencia-project/herencia/blob/master/contrib/gitian-build.py)). Since this is a completely fresh environment, we haven't yet cloned the HERENCIA repository and will need to fetch this script with the following commands:

```bash
curl -L -O https://raw.githubusercontent.com/HERENCIA-Project/HERENCIA/master/contrib/gitian-build.py
...
- Clone the gitian-builder, gitian.sigs, herencia-detached-sigs, and herencia GitHub repos.
