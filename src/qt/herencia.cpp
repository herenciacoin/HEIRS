...
#include "config/herencia-config.h"
#endif
...
#include "qt/herencia/splash.h"
#include "qt/herencia/welcomecontentwidget.h"
...
{
    return QCoreApplication::translate("herencia-core", psz).toStdString();
}
...
static void initTranslations(QTranslator& qtTranslatorBase, QTranslator& qtTranslator, QTranslator& translatorBase, QTranslator& translator, bool forceLangFromSettings = false)
{
    ...
    // Load e.g. bitcoin_de.qm (shortcut "de" needs to be defined in herencia.qrc)
    if (translatorBase.load(lang, ":/translations/"))
        QApplication::installTranslator(&translatorBase);

    // Load e.g. bitcoin_de_DE.qm (shortcut "de_DE" needs to be defined in herencia.qrc)
...
/** Class encapsulating HERENCIA Core startup and shutdown.
...
public:
    ...
    /// Get window identifier of QMainWindow (HERENCIAGUI)
...
{
    window = new HERENCIAGUI(networkStyle, 0);

    pollShutdownTimer = new QTimer(window);
    connect(pollShutdownTimer, &QTimer::timeout, window, &HERENCIAGUI::detectShutdown);
...
void BitcoinApplication::startThread()
{
    ...
    connect(window, &HERENCIAGUI::requestedRestart, executor, &BitcoinCore::restart);
...
void BitcoinApplication::initializeResult(int retval)
{
    ...
    if (retval) {
        ...
        if (pwalletMain) {
            ...
            window->addWallet(HERENCIAGUI::DEFAULT_WALLET, walletModel);
            window->setCurrentWallet(HERENCIAGUI::DEFAULT_WALLET);
        ...
        // HERENCIA: URIs or payment requests:
        //connect(paymentServer, &PaymentServer::receivedPaymentRequest, window, &HERENCIAGUI::handlePaymentRequest);
        connect(window, &HERENCIAGUI::receivedURI, paymentServer, &PaymentServer::handleURIOrFile);
...
{
    QMessageBox::critical(0, "Runaway exception", QObject::tr("A fatal error occurred. HERENCIA can no longer continue safely and will quit.") + QString("\n\n") + message);
    ::exit(1);
}
...
/// 2. Basic Qt initialization (not dependent on parameters or configuration)
    Q_INIT_RESOURCE(herencia_locale);
    Q_INIT_RESOURCE(herencia);
    ...
    /// 6. Determine availability of data directory and parse herencia.conf
    /// - Do not call GetDataDir(true) before this step finishes
    if (!fs::is_directory(GetDataDir(false))) {
        QMessageBox::critical(0, QObject::tr("HERENCIA"),
    ...
    if (!SelectParamsFromCommandLine()) {
        QMessageBox::critical(0, QObject::tr("HERENCIA"), QObject::tr("Error: Invalid combination of -regtest and -testnet."));
        return 1;
    }
#ifdef ENABLE_WALLET
    ...
    // herencia: links repeatedly have their payment requests routed to this process:
...
#if defined(Q_OS_WIN)
        WinShutdownMonitor::registerShutdownBlockReason(QObject::tr("HERENCIA didn't yet exit safely..."), (HWND)app.getMainWinId());
#endif
        app.exec();
        app.requestShutdown();
        app.exec();
