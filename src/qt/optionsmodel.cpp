...
void OptionsModel::setDisplayDefaultOptions(QSettings& settings, bool reset)
{
    if (!settings.contains("nDisplayUnit") || reset)
        settings.setValue("nDisplayUnit", BitcoinUnits::HEIRS);
    ...
    if (settings.contains("nAnonymizeHerenciaAmount") || reset)
        SoftSetArg("-anonymizeherenciaamount", settings.value("nAnonymizeHerenciaAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in herencia.cpp during shotdown to also remove the window positions
