...
#include "qt/herencia/forms/ui_dashboardwidget.h"
#include "qt/herencia/sendconfirmdialog.h"
#include "qt/herencia/txrow.h"
...
DashboardWidget::DashboardWidget(HERENCIAGUI* parent) :
    PWidget(parent),
    ui(new Ui::DashboardWidget)
{
    ...
    // Staking Information
    ui->labelMessage->setText(tr("Amount of HEIRS earned via Staking & Masternodes"));
	...
	ui->labelAmountMNRewards->setText("0 HEIRS");
...
bool hasCharts = false;
    ...
    if (window)
        connect(window, &HERENCIAGUI::windowResizeEvent, this, &DashboardWidget::windowResizeEvent);
#endif
...
void DashboardWidget::loadWalletModel()
{
    if (walletModel && walletModel->getOptionsModel()) {
        ...
        connect(ui->pushImgEmpty, &QPushButton::clicked, window, &HERENCIAGUI::openFAQ);
        connect(ui->btnHowTo, &QPushButton::clicked, window, &HERENCIAGUI::openFAQ);
    ...
    // update the display unit, to not use the default ("HEIRS")
    updateDisplayUnit();
}
...
bool DashboardWidget::loadChartData(bool withMonthNames)
{
    ...
    chartData->amountsByCache = getAmountBy(); // pair HEIRS, zHEIRS
