...
#include "qt/herencia/forms/ui_snackbar.h"
#include "qt/herencia/qtutils.h"
#include <QTimer>


SnackBar::SnackBar(HERENCIAGUI* _window, QWidget *parent) :
    QDialog(parent),
    ui(new Ui::SnackBar),
    window(_window),
    timeout(MIN_TIMEOUT)
{
    ...
    if (window)
        connect(window, &HERENCIAGUI::windowResizeEvent, this, &SnackBar::windowResizeEvent);
    else {
