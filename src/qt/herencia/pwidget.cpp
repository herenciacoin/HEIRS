...
#include "qt/herencia/pwidget.h"
#include "qt/herencia/qtutils.h"
#include "qt/herencia/loadingdialog.h"
#include <QRunnable>
#include <QThreadPool>

PWidget::PWidget(HERENCIAGUI* _window, QWidget *parent) : QWidget((parent) ? parent : _window), window(_window) { init(); }
PWidget::PWidget(PWidget* parent) : QWidget(parent), window(parent->getWindow()) { init(); }

void PWidget::init()
{
    if (window)
        connect(window, &HERENCIAGUI::themeChanged, this, &PWidget::onChangeTheme);
}
